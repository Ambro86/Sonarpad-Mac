...
fn tr(key: &str) -> String {
    tr_map().get(key).cloned().unwrap_or_else(|| key.to_string())
}
...
pub fn menu_label() -> String { tr("audio_description.title") }
pub fn save_folder_label() -> String { tr("audio_description.save_folder") }
...
fn cache_dir(prefix: &str) -> Result<PathBuf, String> {
    ...
    let dir = storage_dir().join("audio_description_cache").join(format!("{prefix}_{}_{}", std::process::id(), stamp));
    fs::create_dir_all(&dir).map_err(|e| format!("Impossibile creare la cache audiodescrizione: {e}"))?;
    Ok(dir)
}
...
fn sanitize_filename(value: &str) -> String {
    ...
    for ch in value.chars() {
        if ch.is_alphanumeric() || matches!(ch, ' ' | '-' | '_' | '.') { out.push(ch); }
        else { out.push('_'); }
    }
    let trimmed = out.trim().trim_matches('.').trim();
    if trimmed.is_empty() { "audiodescrizione".to_string() } else { trimmed.to_string() }
}

fn catalog_path_for_name(name: &str) -> PathBuf {
    catalog_dir().join(format!("{}_character_catalog.json", sanitize_filename(name).replace(' ', "_").to_lowercase()))
}

fn list_catalogs() -> Vec<CharacterCatalog> {
    let mut out = Vec::new();
    let Ok(entries) = fs::read_dir(catalog_dir()) else { return out; };
    for entry in entries.flatten() {
        let path = entry.path();
        if path.extension().and_then(|s| s.to_str()) != Some("json") { continue; }
        let Ok(raw) = fs::read(&path) else { continue; };
        let Ok(file) = serde_json::from_slice::<CharacterCatalogFile>(&raw) else { continue; };
        if file.format != CATALOG_FORMAT { continue; }
        out.push(CharacterCatalog { name: file.name, path, characters: file.characters });
    }
...
fn normalized_catalog_character(character: &BridgeCharacter) -> Option<BridgeCharacter> {
    let name = character.name.split_whitespace().collect::<Vec<_>>().join(" ");
...
fn merge_catalog(existing: &[BridgeCharacter], observed: &[BridgeCharacter]) -> Vec<BridgeCharacter> {
...
fn save_catalog(catalog: &CharacterCatalog, observed: &[BridgeCharacter]) -> Result<PathBuf, String> {
    let merged = merge_catalog(&catalog.characters, observed);
    let created = fs::read(&catalog.path).ok()
        .and_then(|raw| serde_json::from_slice::<CharacterCatalogFile>(&raw).ok())
        .map(|f| f.created_at_utc)
        .unwrap_or_else(now_utc);
    let file = CharacterCatalogFile {
        format: CATALOG_FORMAT.to_string(), version: 1, name: catalog.name.clone(),
        created_at_utc: created, updated_at_utc: now_utc(), characters: merged,
    };
    if let Some(parent) = catalog.path.parent() { fs::create_dir_all(parent).map_err(|e| e.to_string())?; }
    fs::write(&catalog.path, serde_json::to_vec_pretty(&file).map_err(|e| e.to_string())?)
...
fn decode_source_audio(
    ...
    let reader = WavReader::open(wav)
        .map_err(|e| format!("Audio sorgente WAV non leggibile: {e}"))?;
...
fn trim_edge_trailing_silence(samples: &mut Vec<i16>) -> usize {
    ...
    let seek_frames =
        ((MIX_SAMPLE_RATE as u64 * EDGE_TRAILING_SEEK_MS) / 1000).max(1) as usize;
    let window_frames =
        ((MIX_SAMPLE_RATE as u64 * EDGE_TRAILING_WINDOW_MS) / 1000).max(1) as usize;
...
fn synthesize_text_pcm(
    ...
    let mp3 = crate::synthesize_voice_chunk_blocking(
        engine, text, voice, rate, pitch, volume, rt,
    )?;
    let mut pcm = convert_mp3_bytes_to_pcm(&mp3, dir, index, cancel)?;
    if !crate::is_system_voice_engine(engine) {
...
fn normalize_intervals(intervals: &[BridgeInterval], duration: f64) -> Vec<(f64,f64)> {
    let mut values: Vec<_> = intervals.iter().filter_map(|i| {
        let s=i.start_sec.max(0.0).min(duration); let e=i.end_sec.max(s).min(duration); (e>s).then_some((s,e))
    }).collect();
    values.sort_by(|a,b| a.0.total_cmp(&b.0));
    let mut merged: Vec<(f64,f64)> = Vec::new();
    for (s,e) in values { if let Some(last)=merged.last_mut() { if s<=last.1 { last.1=last.1.max(e); continue; } } merged.push((s,e)); }
    merged
}

fn free_intervals(protected: &[(f64,f64)], duration: f64) -> Vec<(f64,f64)> {
    let mut out=Vec::new(); let mut cursor=0.0;
    for &(s,e) in protected { if s>cursor { out.push((cursor,s)); } cursor=cursor.max(e); }
    if cursor<duration { out.push((cursor,duration)); } out
}

fn subtract_reserved(free: &[(f64,f64)], reserved: &[(f64,f64)]) -> Vec<(f64,f64)> {
    let mut blocks=reserved.to_vec(); blocks.sort_by(|a,b| a.0.total_cmp(&b.0));
    let mut out=Vec::new();
    for &(fs_,fe) in free { let mut cur=fs_; for &(rs,re) in &blocks { if re<=cur || rs>=fe {continue;} if rs>cur {out.push((cur,rs.min(fe)));} cur=cur.max(re); if cur>=fe {break;} } if cur<fe {out.push((cur,fe));} }
    out
}

fn choose_slot(free: &[(f64,f64)], desired: f64, required: f64) -> Option<f64> {
    free.iter().filter_map(|&(s,e)| { let upper=e-required; if upper<s {return None;} let x=desired.clamp(s,upper); let d=(x-desired).abs(); (d<=MAX_SHIFT_SEC).then_some((d,x)) }).min_by(|a,b| a.0.total_cmp(&b.0)).map(|x|x.1)
}

fn restrict_slot(free: &[(f64,f64)], d: &SynthesizedDescription) -> Vec<(f64,f64)> {
    let (Some(ss),Some(se))=(d.slot_start_sec,d.slot_end_sec) else {return free.to_vec();};
    free.iter().filter_map(|&(s,e)| {let a=s.max(ss);let b=e.min(se);(b>a).then_some((a,b))}).collect()
}

fn schedule_descriptions(items: &[SynthesizedDescription], protected: &[BridgeInterval], duration: f64, allow_extended: bool) -> (Vec<ScheduledDescription>,Vec<DroppedDescription>) {
    ...
    mandatory.sort_by(|a,b|a.desired_start_sec.total_cmp(&b.desired_start_sec)); optional.sort_by(|a,b|a.desired_start_sec.total_cmp(&b.desired_start_sec));
    mandatory.extend(optional);
    let mut scheduled=Vec::new(); let mut dropped=Vec::new(); let mut reserved=Vec::new();
    for d in mandatory {
        let available=subtract_reserved(&free,&reserved); let candidates=if d.mandatory {restrict_slot(&available,&d)} else {available};
        if let Some(start)=choose_slot(&candidates,d.desired_start_sec,d.duration_sec.max(0.001)) {
            reserved.push((start,start+d.duration_sec)); scheduled.push(ScheduledDescription{original_index:d.original_index,text:d.text,desired_start_sec:d.desired_start_sec,mandatory:d.mandatory,slot_id:d.slot_id,slot_start_sec:d.slot_start_sec,slot_end_sec:d.slot_end_sec,start_sec:start,pcm:d.pcm,duration_sec:d.duration_sec,extended_pause:false}); continue;
        }
        if allow_extended {
            let anchor=candidates.iter().filter_map(|&(s,e)| { if e-s<MIN_EXTENDED_ANCHOR_SEC{return None;} let x=d.desired_start_sec.clamp(s,e-MIN_EXTENDED_ANCHOR_SEC); let dist=(x-d.desired_start_sec).abs();(dist<=MAX_SHIFT_SEC).then_some((dist,x)) }).min_by(|a,b|a.0.total_cmp(&b.0)).map(|x|x.1);
            if let Some(start)=anchor { reserved.push((start,start+MIN_EXTENDED_ANCHOR_SEC)); scheduled.push(ScheduledDescription{original_index:d.original_index,text:d.text,desired_start_sec:d.desired_start_sec,mandatory:d.mandatory,slot_id:d.slot_id,slot_start_sec:d.slot_start_sec,slot_end_sec:d.slot_end_sec,start_sec:start,pcm:d.pcm,duration_sec:d.duration_sec,extended_pause:true}); continue; }
        }
        dropped.push(DroppedDescription{original_index:d.original_index,text:d.text,desired_start_sec:d.desired_start_sec,mandatory:d.mandatory,slot_id:d.slot_id,duration_sec:d.duration_sec,reason:"no dialogue-free slot long enough after exact TTS duration check".to_string()});
    }
    scheduled.sort_by(|a,b|a.start_sec.total_cmp(&b.start_sec)); (scheduled,dropped)
}
...
fn duck_gain_for(frame: u64, start: u64, end: u64, fade_frames: u64) -> f32 {
    ...
    if fade_frames==0 { return if frame>=start && frame<end {duck}else{1.0}; }
    if frame + fade_frames >= start && frame < start { let x=(frame+fade_frames-start) as f32/fade_frames as f32; return 1.0-(1.0-duck)*x.clamp(0.0,1.0); }
    if frame>=start && frame<end { return duck; }
    if frame>=end && frame<end+fade_frames { let x=(frame-end) as f32/fade_frames as f32; return duck+(1.0-duck)*x.clamp(0.0,1.0); }
    1.0
}

fn render_mix(source_wav: &Path, output_wav: &Path, scheduled: &[ScheduledDescription], cancel: &Arc<AtomicBool>) -> Result<f64,String> {
    let mut source=WavReader::open(source_wav).map_err(|e|format!("Apertura audio sorgente fallita: {e}"))?;
    ...
    while source_frame<total_source_frames {
        ...
        while cue_index<scheduled.len() && scheduled[cue_index].extended_pause && (scheduled[cue_index].start_sec*MIX_SAMPLE_RATE as f64).round() as u64<=source_frame {
            let cue=&scheduled[cue_index];
            for &sample in cue.pcm.iter(){writer.write_sample(sample).map_err(|e|e.to_string())?;}
            output_frames += cue.pcm.len() as u64/MIX_CHANNELS as u64; cue_index+=1;
        }
        let left=samples.next().transpose().map_err(|e|e.to_string())?.unwrap_or(0);
        let right=samples.next().transpose().map_err(|e|e.to_string())?.unwrap_or(0);
        let mut narration=[0i16;2]; let mut gain=1.0f32;
        if let Some(cue)=scheduled.get(cue_index).filter(|c|!c.extended_pause) {
            let start=(cue.start_sec*MIX_SAMPLE_RATE as f64).round() as u64; let cue_frames=cue.pcm.len() as u64/MIX_CHANNELS as u64; let end=start+cue_frames;
            gain=duck_gain_for(source_frame,start,end,fade_frames);
            if source_frame>=start && source_frame<end { let off=((source_frame-start)*MIX_CHANNELS as u64) as usize; narration[0]=*cue.pcm.get(off).unwrap_or(&0); narration[1]=*cue.pcm.get(off+1).unwrap_or(&0); }
            if source_frame+1>=end { cue_index+=1; }
        }
        writer.write_sample(mix_sample(left,narration[0],gain)).map_err(|e|e.to_string())?;
        writer.write_sample(mix_sample(right,narration[1],gain)).map_err(|e|e.to_string())?;
        source_frame+=1; output_frames+=1;
    }
    while cue_index<scheduled.len() { let cue=&scheduled[cue_index]; if cue.extended_pause {for &sample in cue.pcm.iter(){writer.write_sample(sample).map_err(|e|e.to_string())?;} output_frames+=cue.pcm.len() as u64/MIX_CHANNELS as u64;} cue_index+=1; }
    writer.finalize().map_err(|e|e.to_string())?;
    Ok(output_frames as f64/MIX_SAMPLE_RATE as f64)
}

fn encode_mp3(wav: &Path, output: &Path, cancel: &Arc<AtomicBool>) -> Result<(),String> {
    let args=vec!["-hide_banner".into(),"-loglevel".into(),"error".into(),"-y".into(),"-i".into(),wav.to_string_lossy().to_string(),"-c:a".into(),"libmp3lame".into(),"-b:a".into(),format!("{BITRATE_KBPS}k"),output.to_string_lossy().to_string()];
    run_ffmpeg(&args,cancel)
}

fn project_path(output: &Path) -> PathBuf { let mut p=output.to_path_buf(); p.set_extension("sonarpad-ad.json"); p }

fn build_project(job:&CreateJob,analysis:&AudioDescriptionBridgeResult,scheduled:&[ScheduledDescription],dropped:&[DroppedDescription],output_duration:f64)->AudioDescriptionProject{
    let mut extra_offset=0.0; let mut descriptions=Vec::new();
    for (id,d) in scheduled.iter().enumerate(){ let output_start=d.start_sec+extra_offset; let ext=if d.extended_pause{d.duration_sec}else{0.0}; let output_end=output_start+d.duration_sec; descriptions.push(ProjectDescription{id,text:d.text.clone(),original_text:d.text.clone(),rendered_text:d.text.clone(),modified:false,gemini_start_sec:d.desired_start_sec,mandatory:d.mandatory,slot_id:d.slot_id.clone(),slot_start_sec:d.slot_start_sec,slot_end_sec:d.slot_end_sec,source_start_sec:d.start_sec,output_start_sec:output_start,output_end_sec:output_end,tts_duration_sec:d.duration_sec,extended_pause:d.extended_pause,extended_pause_duration_sec:ext,duck_start_sec:(!d.extended_pause).then_some(output_start),duck_end_sec:(!d.extended_pause).then_some(output_end)}); extra_offset+=ext; }
    let excluded=dropped.iter().enumerate().map(|(id,d)|ProjectExcluded{id,text:d.text.clone(),gemini_start_sec:d.desired_start_sec,mandatory:d.mandatory,slot_id:d.slot_id.clone(),tts_duration_sec:d.duration_sec,reason:d.reason.clone()}).collect();
    AudioDescriptionProject{format:PROJECT_FORMAT.into(),version:PROJECT_VERSION,created_at_utc:now_utc(),updated_at_utc:now_utc(),source_path:job.input_path.clone(),output_mp3_path:job.output_path.clone(),source_duration_sec:analysis.duration_sec,output_duration_sec:output_duration,language:job.language_code.clone(),language_code:job.language_code.clone(),verbosity:job.verbosity.as_bridge().into(),allow_extended_pauses:job.allow_extended_pauses,recognize_characters:job.recognize_characters,gemini_model:analysis.gemini_model.clone(),tts_engine:job.tts_engine.clone(),tts_voice:job.tts_voice.clone(),tts_rate:job.rate,tts_pitch:job.pitch,tts_volume:job.volume,bitrate_kbps:BITRATE_KBPS,ducking_db:DUCKING_DB,fade_ms:FADE_MS,protected_intervals:analysis.protected_intervals.iter().map(|x|ProjectInterval{start_sec:x.start_sec,end_sec:x.end_sec}).collect(),descriptions,excluded_descriptions:excluded}
}

fn save_project(path:&Path,project:&AudioDescriptionProject)->Result<(),String>{ if let Some(parent)=path.parent(){fs::create_dir_all(parent).map_err(|e|e.to_string())?;} fs::write(path,serde_json::to_vec_pretty(project).map_err(|e|e.to_string())?).map_err(|e|format!("Salvataggio progetto fallito: {e}")) }

fn fetch_gemini_models(api_key:&str)->Result<Vec<String>,String>{
    let key=api_key.trim(); if key.is_empty(){return Err(tr("audio_description.error.api_key"));}
    let url=format!("https://generativelanguage.googleapis.com/v1beta/models?key={}",url::form_urlencoded::byte_serialize(key.as_bytes()).collect::<String>());
    let response=reqwest::blocking::Client::builder().timeout(Duration::from_secs(30)).build().map_err(|e|e.to_string())?.get(url).send().map_err(|e|e.to_string())?;
    if !response.status().is_success(){return Err(format!("HTTP {}",response.status()));}
    let root:Value=response.json().map_err(|e|e.to_string())?; let mut out=Vec::new();
    if let Some(models)=root.get("models").and_then(Value::as_array){ for model in models { let methods=model.get("supportedGenerationMethods").and_then(Value::as_array); if !methods.is_some_and(|m|m.iter().any(|x|x.as_str()==Some("generateContent"))){continue;} if let Some(name)=model.get("name").and_then(Value::as_str){out.push(name.trim_start_matches("models/").to_string());} } }
    out.sort(); out.dedup(); if out.is_empty(){Err("Nessun modello Gemini compatibile trovato.".into())}else{Ok(out)}
}

fn create_audio_description(job:&CreateJob,rt:&Runtime,cancel:Arc<AtomicBool>,state:Arc<Mutex<ProgressState>>)->Result<JobOutcome,String>{
    if !job.input_path.is_file(){return Err(tr("audio_description.error.input"));}
    if job.output_path.as_os_str().is_empty(){return Err(tr("audio_description.error.output"));}
    ...
    let result=(||{
        {let mut s=state.lock().unwrap();s.progress=2;s.status=tr("audio_description.progress.analysis_prepare");}
        ...
        for (i,d) in analysis.descriptions.iter().enumerate(){if cancel.load(Ordering::Relaxed){return Err("cancelled".into());} let pcm=synthesize_text_pcm(&d.text,&job.tts_engine,&job.tts_voice,job.rate,job.pitch,job.volume,rt,&work,i,&cancel)?; let duration_sec=pcm.len() as f64/(MIX_CHANNELS as f64*MIX_SAMPLE_RATE as f64); synthesized.push(SynthesizedDescription{original_index:i,text:d.text.clone(),desired_start_sec:d.start_sec,mandatory:d.mandatory,slot_id:d.slot_id.clone(),slot_start_sec:d.slot_start_sec,slot_end_sec:d.slot_end_sec,pcm,duration_sec}); let mut s=state.lock().unwrap();s.progress=55+(((i+1)as i32*25)/(analysis.descriptions.len().max(1)as i32));}
        {let mut s=state.lock().unwrap();s.status=tr("audio_description.progress.schedule");s.progress=80;}
        let(scheduled,dropped)=schedule_descriptions(&synthesized,&analysis.protected_intervals,analysis.duration_sec,job.allow_extended_pauses); if scheduled.is_empty(){return Err("Nessuna descrizione può essere inserita in sicurezza.".into());}
        let mix_wav=work.join("mix.wav"); let output_duration=render_mix(&source_wav,&mix_wav,&scheduled,&cancel)?;
        {let mut s=state.lock().unwrap();s.status=tr("audio_description.progress.export");s.progress=90;}
        let temp_output=work.join("final.mp3"); encode_mp3(&mix_wav,&temp_output,&cancel)?; fs::copy(&temp_output,&job.output_path).map_err(|e|format!("Salvataggio MP3 fallito: {e}"))?;
        let project_file=if job.save_project{let p=project_path(&job.output_path);let project=build_project(job,&analysis,&scheduled,&dropped,output_duration);save_project(&p,&project)?;Some(p)}else{None};
        let catalog_path=if job.keep_character_catalog{if let Some(c)=job.catalog.as_ref(){Some(save_catalog(c,&analysis.character_glossary)?)}else{None}}else{None};
        let extended=scheduled.iter().filter(|x|x.extended_pause).count(); let dropped_mandatory=dropped.iter().filter(|x|x.mandatory).count();
        Ok(JobOutcome{output_path:job.output_path.clone(),project_path:project_file,catalog_path,generated:analysis.descriptions.len(),inserted:scheduled.len(),extended,dropped:dropped.len(),dropped_mandatory})
    })();
...
fn quota_dialog(parent:&Frame,model:&str,error:&str,api_key:&str)->AudioDescriptionQuotaDecision{
    let d=Dialog::builder(parent,&tr("audio_description.quota.title")).with_size(620,300).build();let p=Panel::builder(&d).build();let root=BoxSizer::builder(Orientation::Vertical).build();let msg=trf("audio_description.quota.message",&[("model",model.to_string()),("error",error.to_string())]);root.add(&StaticText::builder(&p).with_label(&msg).build(),0,SizerFlag::Expand|SizerFlag::All,8);let choice=Choice::builder(&p).build();let models=fetch_gemini_models(api_key).unwrap_or_default().into_iter().filter(|m|m!=model).collect::<Vec<_>>();for m in &models{choice.append(m);}if !models.is_empty(){choice.set_selection(0);}root.add(&choice,0,SizerFlag::Expand|SizerFlag::All,8);let row=BoxSizer::builder(Orientation::Horizontal).build();let sw=Button::builder(&p).with_id(7001).with_label(&tr("audio_description.quota.model_prompt")).build();let wait=Button::builder(&p).with_id(7002).with_label(&tr("audio_description.quota.wait")).build();let stop=Button::builder(&p).with_id(ID_CANCEL).with_label(&tr("audio_description.cancel")).build();row.add(&sw,0,SizerFlag::All,5);row.add(&wait,0,SizerFlag::All,5);row.add(&stop,0,SizerFlag::All,5);root.add_sizer(&row,0,SizerFlag::Expand,0);p.set_sizer(root,true);let d1=d;sw.on_click(move |_|d1.end_modal(7001));let d2=d;wait.on_click(move |_|d2.end_modal(7002));let d3=d;stop.on_click(move |_|d3.end_modal(ID_CANCEL));let r=d.show_modal();let result=if r==7001{models.get(choice.get_selection().unwrap_or(0)as usize).cloned().map(AudioDescriptionQuotaDecision::SwitchModel).unwrap_or(AudioDescriptionQuotaDecision::Wait)}else if r==7002{AudioDescriptionQuotaDecision::Wait}else{AudioDescriptionQuotaDecision::Stop};d.destroy();result
}

fn run_with_progress(parent:&Frame,job:CreateJob,rt:Arc<Runtime>)->Result<JobOutcome,String>{
    let progress=ProgressDialog::builder(parent,&tr("audio_description.title"),&tr("audio_description.status.running"),100).with_style(ProgressDialogStyle::CanAbort|ProgressDialogStyle::Smooth).build();
    crate::set_progress_cancel_label(&progress);
    let state=Arc::new(Mutex::new(ProgressState{progress:0,status:tr("audio_description.status.running"),done:None,quota:None}));let cancel=Arc::new(AtomicBool::new(false));let st=state.clone();let c=cancel.clone();let job_thread=job.clone();thread::spawn(move||{let r=create_audio_description(&job_thread,&rt,c,st.clone());st.lock().unwrap().done=Some(r);});
    loop{thread::sleep(Duration::from_millis(100));let snap=state.lock().unwrap().clone();if let Some(q)=snap.quota{state.lock().unwrap().quota=None;let decision=quota_dialog(parent,&q.model,&q.error,&job.gemini_api_key);let _=q.sender.send(decision);}if let Some(done)=snap.done{progress.destroy();return done;}if !progress.update(snap.progress.clamp(0,99),Some(&snap.status)){cancel.store(true,Ordering::SeqCst);state.lock().unwrap().status=tr("audio_description.status.canceling");}crate::set_progress_cancel_label(&progress);}
}

fn language_choices()->Vec<(&'static str,&'static str)>{vec![("Italiano","it"),("English","en"),("Deutsch","de"),("Español","es"),("Français","fr"),("Português","pt"),("Português (Brasil)","pt-BR"),("Čeština","cs"),("Polski","pl"),("Русский","ru"),("Українська","uk"),("Svenska","sv"),("Tiếng Việt","vi"),("中文","zh"),("हिन्दी","hi")]}

fn voice_matches_language(voice:&VoiceInfo,language:&str)->bool{let wanted=language.split('-').next().unwrap_or(language).to_ascii_lowercase();voice.locale.split('-').next().unwrap_or(&voice.locale).eq_ignore_ascii_case(&wanted)}

pub fn open_create_dialog(parent:&Frame,settings:&Arc<Mutex<Settings>>,rt:&Arc<Runtime>,voices_data:&Arc<Mutex<Vec<VoiceInfo>>>){
    let saved=settings.lock().unwrap().clone();let d=Dialog::builder(parent,&tr("audio_description.title")).with_style(DialogStyle::DefaultDialogStyle|DialogStyle::ResizeBorder).with_size(760,680).build();let p=Panel::builder(&d).build();let root=BoxSizer::builder(Orientation::Vertical).build();
    let input=TextCtrl::builder(&p).build();let input_row=BoxSizer::builder(Orientation::Horizontal).build();input_row.add(&StaticText::builder(&p).with_label(&tr("audio_description.input")).build(),0,SizerFlag::AlignCenterVertical|SizerFlag::All,5);input_row.add(&input,1,SizerFlag::Expand|SizerFlag::All,5);let input_btn=Button::builder(&p).with_label(&tr("audio_description.browse")).build();input_row.add(&input_btn,0,SizerFlag::All,5);root.add_sizer(&input_row,0,SizerFlag::Expand,0);
    let output=TextCtrl::builder(&p).build();let output_row=BoxSizer::builder(Orientation::Horizontal).build();output_row.add(&StaticText::builder(&p).with_label(&tr("audio_description.output")).build(),0,SizerFlag::AlignCenterVertical|SizerFlag::All,5);output_row.add(&output,1,SizerFlag::Expand|SizerFlag::All,5);let output_btn=Button::builder(&p).with_label(&tr("audio_description.browse")).build();output_row.add(&output_btn,0,SizerFlag::All,5);root.add_sizer(&output_row,0,SizerFlag::Expand,0);
    let language=Choice::builder(&p).build();let langs=language_choices();for (name,_) in &langs{language.append(name);}let lang_index=langs.iter().position(|(_,code)|*code==saved.audio_description_language).unwrap_or(0);language.set_selection(lang_index as u32);let row=BoxSizer::builder(Orientation::Horizontal).build();row.add(&StaticText::builder(&p).with_label(&tr("audio_description.language")).build(),0,SizerFlag::AlignCenterVertical|SizerFlag::All,5);row.add(&language,1,SizerFlag::Expand|SizerFlag::All,5);root.add_sizer(&row,0,SizerFlag::Expand,0);
    ...
    let d_input=d;input_btn.on_click(move |_|{if let Some(path)=choose_input(&d_input){input.set_value(&path.to_string_lossy());if output.get_value().trim().is_empty(){let mut dest=default_output_dir().join(path.file_stem().and_then(|s|s.to_str()).unwrap_or("video"));dest.set_extension("mp3");let stem=path.file_stem().and_then(|s|s.to_str()).unwrap_or("video");dest=default_output_dir().join(format!("{}_audiodescritto.mp3",sanitize_filename(stem)));output.set_value(&dest.to_string_lossy());}}});
    ...
    let parent_modify=*parent;let rt_modify=Arc::clone(rt);let voices_modify=Arc::clone(voices_data);modify.on_click(move |_|open_project_editor(&parent_modify,&rt_modify,&voices_modify));
...
fn project_edit_available_duration(
    ...
    let Some((_, gap_end)) = free.iter().find(|(gap_start, gap_end)| {
        start + 0.001 >= *gap_start && start <= *gap_end + 0.001
    }) else {
        return Ok(Some(0.0));
    };
...
fn rebuild_project(
    ...
    let result = (|| {
        ...
        for (index, description) in project.descriptions.iter().enumerate() {
            ...
            state.lock().unwrap().progress = 10
                + ((index + 1) as i32 * 70 / project.descriptions.len().max(1) as i32);
        }
...
pub fn open_project_editor(
    ...
    choice.on_selection_changed(move |_| {
        if let Some(index) = choice.get_selection().map(|value| value as usize) {
            if let Some(description) = project_selection.borrow().descriptions.get(index) {
                text.set_value(&description.text);
                status.set_label(&project_description_details(&project_selection.borrow(), index));
            }
        }
    });
    ...
    delete.on_click(move |_| {
        let Some(index) = choice.get_selection().map(|value| value as usize) else {
            show_error(&dialog_delete, &tr("audio_description.project.no_selection"));
            return;
        };
    ...
    export.on_click(move |_| {
        ...
        let progress = ProgressDialog::builder(
            &parent_export,
            &tr("audio_description.project.title"),
            &tr("audio_description.project.status.exporting"),
            100,
        )
        ...
        let cancel = Arc::new(AtomicBool::new(false));
        let mut project_copy = project_export.borrow().clone();
        let thread_state = Arc::clone(&state);
        let thread_cancel = Arc::clone(&cancel);
        let project_file = path_export.clone();
        let runtime = Arc::clone(&rt_export);
        thread::spawn(move || {
        ...
        loop {
            ...
            if let Some(result) = snapshot.done {
                progress.destroy();
                match result {
                    Ok(_) => {
                        if let Ok(updated) = load_project(&path_export) {
                            *project_export.borrow_mut() = updated;
                        }
                        choice.clear();
                        for description in &project_export.borrow().descriptions {
                            choice.append(&format!(
                                "{} - {}",
                                format_mmss(description.source_start_sec),
                                description.text
                        ...
                        if !project_export.borrow().descriptions.is_empty() {
                            choice.set_selection(0);
                            text.set_value(&project_export.borrow().descriptions[0].text);
                            status.set_label(&project_description_details(
                                &project_export.borrow(),
                                0,
                            ));
                        }
                        show_info(&dialog_export, &tr("audio_description.project.success"));
                    }
                    Err(error) => show_error(&dialog_export, &error),
                }
                break;
            }
            if !progress.update(
                snapshot.progress.clamp(0, 99),
                Some(&tr("audio_description.project.status.exporting")),
            ) {
    ...
    close.on_click(move |_| dialog_close.end_modal(ID_CANCEL));
    dialog.show_modal();
    dialog.destroy();
}
