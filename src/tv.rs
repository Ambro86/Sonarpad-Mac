...
fn append_current_programs(channels: &mut [TvChannel]) {
    let Ok(current_programs) = fetch_current_programs() else {
        return;
    };
    for channel in channels {
        let key = normalize_oggi_in_tv_channel_name(&channel.name);
        if let Some(program) = current_programs.get(&key) {
            channel.current_program = Some(program.clone());
        }
    }
}

fn fetch_current_programs() -> Result<HashMap<String, String>, String> {
    ...
    for group in timeline {
        for program in group {
            if program.start_time <= now && now < program.end_time {
